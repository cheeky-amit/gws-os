...
**Phase 3: Graph + Calendar (Days 8-12)**
- Introduce `graph.jsonl` and Python graph scripts (`bin/gws-graph-read.py`, `bin/gws-graph-write.py`, `bin/gws-graph-compact.py`)
- Build graph edge weighting, confidence scoring, and the graph query interface
...
- **Phase 3 tests**: graph read/write/compact scripts, edge weighting, cross-calendar scheduling, brief assembly

**Phase 4: Learning Engine + Polish (Days 13-18)**
- Implement trust progression/demotion logic (streaks, promotion thresholds)
...
- **Phase 4 tests**: trust progression/demotion, pattern detection, graph compaction, weekly/learn skills
